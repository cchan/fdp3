...
WHiL - Waterloop's Hardware in the Loop testing software \\
    ...
    \subsection{Main Embedded Board (MEB)}
...
blown chip would have resulted in a waste of said resources. Thus it was decided to be more efficient to simply mount a OTS RPi onto the PCB as described above.
    ...
    \subsection{Hub Processing Boards (HPB)}
...
\subsection{Sensor and Control Hubs}
\label{subsec:sensor-and-control-hubs}
...
\subsubsection{Sensors/Hubs Map}
As described in \refsubsec{subsec:sensor-and-control-hubs}, all of the sensor and control data will be centralized and passed through Hubs on-board of the pod. With the importance of placement of electronic components on the pod, \reffig{fig:sensor-map} describes the mapping of all mounting locations.
  ...
  \begin{figure}
   ...
   \subsubsection{HiL testing description}
...
When the function returns $[1:-1]$ (with x as the sensor input value) there exists an extreme state. A full list of functions is given in \reftab{tab:hil-functions}.
    ...
    \subsubsection{HiL testing setup}
...
\subsection{Embedded Bill of Materials}
