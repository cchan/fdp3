...
\begin{document}
    ...
    A full Bill of Materials can be found in \reftab{tab:lateral-bom}.
    ...
    \begin{figure}
        ...
        \caption{The plot of the distance to the rail in when the rails are arranged in a random order. We see that the lowest pod-rail distance is less than 12 millimetres; for reference, the brakes are nominally at \SI{19}{mm} when deployed.}
    \end{figure}
    MATLAB simulations were created to ensure the lateral control system would maintain a close distance to the rail so as to prevent any pod component from entering a no-go zone or colliding with the rail. For example, the Eddy Current Brakes (when deployed) have a nominal distance to the rail of \SI{19}{mm}. As such, the lateral control system must always maintain a distance smaller than \SI{19}{mm}, seen on Figure 5.2 on page 63.\\
    ...
    \begin{figure}
        ...
        \caption{This plot is similar to Figure 5.2 on page 66 except that the arrangement of the rail in this simulation is in alternating order. The closest distance is again \SI{12}{mm}.}
    \end{figure}
    \begin{figure}[H]
    	\centering
