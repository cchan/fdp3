...
\end{table}
    ...
    By the physical nature of the CAN-BUS requiring two wires for comand being based bus topology if a bus wire is disconnected or breaks, the bus will lose all communications. If a device is disconnected from the bus this can cause errors as a CAN node that doesn't receive any acknowledgment for a packet it has sent, it will keep trying to send it forever, hence clogging the bus. Both cases will trigger a failure mode as heartbeats will not be received stopping the pod. Even with these failure cases the pod will still be able to come to a controlled stop. 
